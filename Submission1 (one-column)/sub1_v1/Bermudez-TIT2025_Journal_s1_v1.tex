...
%\begin{definition} [Information]\label{DefInformation} 
...
%\begin{equation}
%\label{EqDefInfo}
%\iota_{XY}(x,y) = -\log\left( \RND{P_XY}{Q}(x, y) \right) .
%\end{equation}
%\end{definition}
...
\begin{definition}[Entropy]\label{DefEnt} Let $X$ be a discrete random variable and assume it  induces the probability measure $P_X$ \in \triangle(\set{X}, \set{F}_\set{X})$ and a sigma finite probability measure $Q \in \triangle(\set{X}, \set{F}_\set{X})$, with $P_X\ll Q$,
 on $\left( \mathds{R}, \mathcal{B} \left( \mathds{R} \right) \right)$. Then, the entropy of $X$, denoted by $H(X)$, $H(\RND{P_X}{Q}(x))$ or $H(P_X)$, is:
\begin{equation}
\label{EqDiscreteEntropy}
H(X) \triangleq   - \sum_{x \in \supp{P_X} } \hspace{-2mm} \RND{P_X}{Q}(x) \log P_X(x).
\end{equation}
When $Q$ is the counting measure $\RND{P_X}{Q}$ is the probability mass function induced by $P_X$, and when $Q$ is the Lebesgue measure $\RND{P_X}{Q}$ is the probability density function induced by $P_X$.
\end{definition}
